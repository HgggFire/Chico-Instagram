        ...
        FirebaseCall.sharedInstance().getAllPosts { (data, err) in
            ...
            for (id, postDict) in dict {
                    ...
                    let description = pDict["description"] as? String
                {
                    ...
                    let feed = HomeFeed(id: id, description: description, likeCount: likeCount, isLiked: isLiked, uid: uid, timestamp: date)
                    feeds.append(feed)
                }
            }
...
extension HomeFeedViewController: CreatePostViewControllerDelegate {
    func didPost(image: UIImage, text: String) {
        FirebaseCall.sharedInstance().createPost(withImage: image, description: text) { (key, err) in
            if err == nil {
                self.loadPage()
